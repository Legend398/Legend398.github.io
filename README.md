...
A software-engineering portfolio built from verified CV and project evidence. The homepage pairs plain-language project stories and genuine product media with one readable `BUILD` glass word powered by Three.js. The word stays sharp at rest and applies a small optical frost only where the pointer intersects it; reduced-motion and no-WebGL sessions receive a readable CSS fallback.
