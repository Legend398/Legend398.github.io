...
// Adapted from React Bits Liquid Ether; see THIRD_PARTY_NOTICES.md.

/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unused-vars -- upstream React Bits simulation internals */
...
`;
  ...
  return <div ref={mountRef} className={[styles.root, className].filter(Boolean).join(' ')} data-liquid-ether data-mode="webgl" aria-hidden="true" style={style} />;
}
...
export default function LiquidEther(props: LiquidEtherProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      setMotionAllowed(Boolean(context));
  ...
  if (!motionAllowed) {
    return (
      <div
        className={[styles.root, styles.fallback, props.className].filter(Boolean).join(' ')}
        data-liquid-ether
        data-mode="css"
        aria-hidden="true"
        style={props.style}
  ...
  return <LiquidEtherCanvas {...props} />;
}
